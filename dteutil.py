...
def calculate_expiry_date(days_from_today: int) -> str:
    market_calendar = mcal.get_calendar('NYSE')  # Use the appropriate calendar
    today = datetime.now()
    ...
    # Normalize expiry_date to midnight UTC
    utc_tz = pytz.UTC
    expiry_date = datetime(expiry_date.year, expiry_date.month, expiry_date.day, 0, 0, 0, tzinfo=utc_tz)

    start_date = expiry_date.strftime('%Y-%m-%d')
    end_date = (expiry_date + timedelta(days=30)).strftime('%Y-%m-%d')
    valid_days = market_calendar.valid_days(start_date=start_date, end_date=end_date)

    # Debug
    #print(f"Valid trading days between {start_date} and {end_date}: {valid_days}")

    # Check if expiry_date is valid, increment day by day until found
    while expiry_date not in valid_days:
        expiry_date += timedelta(days=1)
        if expiry_date > utc_tz.localize(today + timedelta(days=30)):
            raise ValueError("Could not find a valid trading day within 30 days")
